...
void EEpromSaveInit()
{
   ...
   EeReadTrg(&EeSave1);
   EePromDeal(&DianXin1.Save, sizeof(SaveData) - CRC_LEN, &EeSave1);
   if (EeSave1.Solo.BadSave)
   {
      EeSave1.Solo.BadSave = 0;
      DianXin1.Save.Solo.SaveTest = 10;
   }
...
void EEpromWrite()
{
   uint16 tmpCrc = DianXin1.Save.Solo.Crc;
   uint16 tmpCrcNew = GetCRC16((uint08 *)&DianXin1.Save, sizeof(SaveData) - CRC_LEN);
   if (tmpCrc != tmpCrcNew)
   {
