...
void main(void)
{
   ...
   while (1)
   {
      ...
      {
         EEpromTimeOut();
         EePromDeal(&MODE1.SaveData, sizeof(Module_Save) - CRC_LEN, &EeSave1);
         OutputDeal(&MODE1);
      }
      if (Timer1.Flags.Solo._20ms)
      {
         AdcDeal(&MODE1);
      }
      ...
      MODE1.DO.Solo.LED1 = Timer1.Flags.Solo.Blink;
      Timer1.Flags.All[0] = 0;
...

{
   ...
   if (sel == 1)
   {
      ...
      scanf("%s", str1);
      r = l->next;
      while (r != NULL) if (strcmp(r->data.num, str1) == 0) /*检索是否有与输入的车号相匹配的*/
         {
            s[i] = r;
            i++;
            break;
   ...
   else if (sel == 2)
   {
      ...
      scanf("%s", str2);
      r = l->next;
      while (r != NULL) if (strcmp(r->data.reachcity, str2) == 0) /*检索是否有与输入的城市相匹配的火车*/
         {
            s[i] = r;
            i++;
            r = r->next;
   ...
   if (i == 0) printf("can not find!");
   else
   {
      printheader();
      for (k = 0; k < i; k++) printdata(s[k]);
   }
}
...
void Bookticket(Link l, bookLink k)
{
   ...
   else
   {
      ...
      if (strcmp(ch, "Y") == 0 || strcmp(ch, "y") == 0) /*判断是否订票*/
      {
         ...
         scanf("%s", tnum);
         for (t = 0; t < i; t++) if (strcmp(r[t]->data.num, tnum) == 0)
            {
...
void Modify(Link l)
{
   ...
   else
   {
      ...
      if (ch == 'y' || ch == 'Y')
      {
         ...
         scanf("%s", tnum);
         while (p != NULL) if (strcmp(p->data.num, tnum) == 0) /*查找与输入的车号相匹配的记录*/
               break;
            else p = p->next;
         if (p)
         {
...
void showtrain(Link l) /*自定义函数显示列车信息*/
{
   ...
   p = l->next;
   printheader();
   if (l->next == NULL) printf("no records!");
   else while (p != NULL)
      {
         printdata(p);
         p = p->next;
