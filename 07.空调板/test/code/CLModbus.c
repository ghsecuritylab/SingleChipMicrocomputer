...
MdErrEnum MdDataDeal(Md_Tcp_Dep *Tcp)
{
   ...
   MdErrEnum ret;
   MyMdAddr = FC1.State.DI.Solo.Addr & 0x003F;
//  1. 地址判断
#define MD_DEV_ADDR_CHK
   if (Tcp->RawData[MD_DEV_ADDR_INDEX] != MyMdAddr && Tcp->RawData[MD_DEV_ADDR_INDEX] != 0xFF)
   {
...
#define _MODBUS_MASTER_0X10
   else
   {
      ...
      Tcp->p2trg[3] = Tcp->NowPoll.StartAddr % 256;
      Tcp->p2trg[4] = 0x00;
      Tcp->p2trg[5] = Tcp->NowPoll.DataToRead / 2;
      Tcp->p2trg[6] = Tcp->NowPoll.DataToRead;
      ...
      if (Tcp->RdData != NULL)
      {
         for (tmp = 0; tmp < length; tmp++)
         {
            itmp = Tcp->RdData(Tcp->NowPoll.StartAddr + tmp);
            Tcp->p2trg[tmpindex++] = itmp / 256;
            Tcp->p2trg[tmpindex++] = itmp % 256;
         }
      }
